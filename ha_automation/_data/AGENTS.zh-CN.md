...
### 4. 先确认设备与功能语义，再写脚本（强制）

- 在新增/修改任何自动化脚本前，**必须先查设备列表**确认用户提到的设备
- 至少执行一次：`ha-automation devices "关键词"`（必要时多个关键词）
- 对候选实体，必须再确认：
  - `entity_id` 是否正确
  - `domain` 是否匹配（如 `sensor` / `switch` / `binary_sensor`）
  - `state` 语义是否匹配业务意图（如“明亮”到底是 `强`、`on` 还是数值阈值）
- 未确认前，不要凭名称猜测直接写条件
- 名称歧义或多设备重名时，脚本内直接硬编码已确认 `entity_id`，并加注释说明
